...
public class AccDetailsFromAddr extends LatLng2Poi{
		
	public static void main(String[] args){
		
		// store the details for each account in file as well as for each poi in map
		accountDetailsFromAddress();
		
		File d_file = new File(comp(out_file));
		d_file.delete();
		
		File r_file = new File(out_file);
		File n_file = new File("addr_acc_details");
		r_file.renameTo(n_file);
		
		//display final file name in which result is stored
		System.out.println("completed : " + out_file + " , total addresses: " + Integer.toString(total_addr) + " , poi_addresses: " + Integer.toString(poi_addr));
	...
	public static void accountDetailsFromAddress(){
		try{
			...
			initializeFile(out_file);
			initializeFile(out_file + "_2");
	    	...
	    	for(String line; ((line = br.readLine()) != null) && (it < 200); ) {
				...
				if(!geocode_result_obj.has("message")){
					...
					FileReader fr = new FileReader(out_file);
		            BufferedReader br1 = new BufferedReader(fr);
		            FileWriter fw = new FileWriter(comp(out_file));
